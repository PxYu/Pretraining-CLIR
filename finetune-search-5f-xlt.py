...
class CLIR:
    ...
    def __init__(self, model, fold):
        ...
        if args.dataset == 'clef':
            ...
            split = pickle.load(open(f"{data_dir}/relevance/{args.target_lang}_split_5f.pkl", 'rb'))
            ...
            train_idx = [(self.fold - 1) % 5 + 1, self.fold % 5 + 1, (self.fold + 1) % 5 + 1]
            train_idx = [f"f{x}" for x in train_idx]
            for i in train_idx:
                alter_split['train'].extend(split[i])

            dev_idx = (self.fold + 2) % 5 + 1
            dev_idx = f"f{dev_idx}"
            alter_split['dev'].extend(split[dev_idx])

            test_idx = (self.fold + 3) % 5 + 1
            test_idx = f"f{test_idx}"
            alter_split['test'].extend(split[test_idx])
        ...
        # dataloaders
        train_dataset = Retrieval_Trainset(id2query=queries, id2doc=documents, rel=rel, split=alter_split, num_neg=args.num_neg, neg_value=0)
        ...
        dev_dataset = Retrieval_Testset(id2query=queries, id2doc=documents,rel=rel, split=alter_split, mode="dev", neg_value=0)
        ...
        test_dataset = Retrieval_Testset(id2query=queries, id2doc=documents,rel=rel, split=alter_split, mode="test", neg_value=0)
        ...
        self.test_len = len(self.test_loader.dataset.query_ids)
        logger.info(f"test has {self.test_len} queries ...")
    ...
    def run(self):
        ...
        for epoch in range(args.num_epochs):
            ...
            if self.epoch % self.eval_interval == 0 and self.epoch >= int(args.num_epochs/2):
        ...
        logger.info("[fold {}] best test MAP: {:.3f} @ epoch {}".format(self.fold, self.best_test_map, self.best_epoch))

        return self.best_test_map, self.test_len
    ...
    def eval(self, splt):
        ...
        os.makedirs("tmp_5f", exist_ok=True)
        record_path = f"tmp_5f/{args.model_type}_{args.source_lang}{args.target_lang}_{splt}_{self._rank}_{self.epoch}.txt"
        ...
        for rank in range(get_world_size()):
            log_path = f"tmp_5f/{args.model_type}_{args.source_lang}{args.target_lang}_{splt}_{rank}_{self.epoch}.txt"
        ...
        if self._rank == 0:
            ...
            for file in glob.glob(f"tmp_5f/{args.model_type}_{args.source_lang}{args.target_lang}_{splt}*{self.epoch}.txt"):
...
for fold in [1,2,3,4,5]:
    clir = CLIR(model, fold)
    t_map, t_len = clir.run()
    maps.append(t_map)
    lens.append(t_len)

logger.info(maps)
logger.info(lens)
avg_map = sum(x * y for x, y in zip(maps, lens)) / sum(lens)
logger.info(f"final MAP: {avg_map:.3f}")
